...
namespace Ludo
{
    public class LudoHomeController : MonoBehaviour
    {
        ...
        public void UserLeft() => innerHome.SetActive(false);
        ...
        public void UpdateUserScore(int score)
        {
            if (gameManager.gamePlayMode == GamePlayMode.Online)
                ...
                scoreText.text = OfflineScore().ToString();
        }
        ...
        {
            if (gameManager.gamePlayMode == GamePlayMode.Online)
                gameManager.socketConnection.SendDataToSocket(gameManager.socketEventManager.SendDiceAnimation(), DiceAnimationStartAcknowledgement, LudoNumberEventList.DICE_ANIMATION_STARTED.ToString());
            else
                gameManager.aiController.DiceAnimation();
        }
        ...
        public void TurnTimeStart()
        {
            ...
            if (turnTimerImage.fillAmount <= 0)
            {
                ...
                {
                    if (!isUserTurnFinish)
                        gameManager.aiController.AiUserExtraTurn();
                    else if (!isUserExtraTurnFinish)
                        gameManager.aiController.AiUserTurnMissed();
                }
            }
        }
        ...
        [Header("============ OFFLINE MODE VARIABLE ============")]
        public bool isUserTurnFinish;
        public bool isUserExtraTurnFinish;
        public List<int> playersMoves;
    }
}
