...
namespace Ludo
{
    public class LudoEmojiController : MonoBehaviour
    {
        ...
        public void ClickedOnEmojiIndex(int emojiNo)
        {
            if (gameManager.gamePlayMode == GamePlayMode.Online)
                gameManager.socketConnection.SendDataToSocket(gameManager.socketEventManager.EmojiRequestData(emojiNo, selectToSendIndex), EmojiAcknowledgement, LudoNumberEventList.EMOJI.ToString());
            else
                gameManager.aiController.SendEmoji(gameManager.socketEventManager.EmojiRequestData(emojiNo, selectToSendIndex));
