...
namespace Ludo
{
    public class LudoTokenController : MonoBehaviour, IPointerDownHandler
    {
        ...
        private void Awake()
        {
            ...
            if (!gameManager.signUpRequestSDKData.gameModeName.Equals("CLASSIC"))
            {
                ...
                tokenRectTransfrom.DOJumpAnchorPos(new Vector3(0, 30, 0), jumpPower, jumpnumber, moveTime).OnComplete(() =>
                {
                    ...
                    playersWayPoints.wayPointsForTokenMove[myLastBoxIndex].GetComponent<LudoBoxProperties>().UpdateMyColor(myColor);
                    SoundManager.instance.SoundPlay(SoundManager.instance.tokenMoveAudio);
                    DoFade(playersWayPoints.wayPointsForTokenMove[myLastBoxIndex].GetComponent<LudoBoxProperties>().trail);
                    CoockieManage();
        ...
        {
            Debug.Log("I AH HERE ============================");
            if (gameManager.gamePlayMode == GamePlayMode.Online)
            {
                if (gameManager.signUpRequestSDKData.gameModeName.Equals("CLASSIC"))
                {
                    MoveToken();
            ...
            else if (gameManager.gamePlayMode == GamePlayMode.Offline)
            {
                gameManager.aiController.AiMoveToken(tokenIndex);
            }
        }
        ...
        IEnumerator Movement(int movementValue)
        {
            ...
            CoockieManage();
            if (playersWayPoints.wayPointsForTokenMove[myLastBoxIndex].GetComponent<LudoBoxProperties>().boxType == BoxType.Star)
            ...
            {
                Debug.LogError("==========> " + playersWayPoints.wayPointsForTokenMove[myLastBoxIndex].GetComponent<LudoBoxProperties>());
                gameManager.aiController.CheckForTokenkill(this, playersWayPoints.wayPointsForTokenMove[myLastBoxIndex].GetComponent<LudoBoxProperties>());
            }
        ...
        public void CoockieManage()
        {
            for (int i = 0; i < playersWayPoints.wayPointsForTokenMove.Count; i++)
                playersWayPoints.wayPointsForTokenMove[i].GetComponent<LudoBoxProperties>().CockieManage();
        }
        ...
        public void SetMyPositionOnRejoin(int myBoxIndex)
        {
            ...
            {
                SoundManager.instance.TimeSoundStop(SoundManager.instance.timerAudio);
                playersWayPoints.wayPointsForTokenMove[myBoxIndex].GetComponent<LudoBoxProperties>().UpdateMyColor(myColor);
                SoundManager.instance.SoundPlay(SoundManager.instance.tokenMoveAudio);
                DoFade(playersWayPoints.wayPointsForTokenMove[myBoxIndex].GetComponent<LudoBoxProperties>().trail);
                CoockieManage();
            });
        ...
        public IEnumerator KillMovement()
        {
            ...
            if (gameManager.gamePlayMode == GamePlayMode.Offline)
                gameManager.aiController.OnKillOffline();
        }
