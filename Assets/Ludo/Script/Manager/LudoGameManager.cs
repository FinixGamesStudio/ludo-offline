...
namespace Ludo
{
    public enum GamePlayMode { None, Online, Offline, FTUE }
    ...
    public class LudoGameManager : MonoBehaviour
    {
        ...
        [Header("Ludo AI Controller")]
        public LudoAiController aiController;
        public LudoOfflineManager offlineManager;
        ...
        public LudoTokenController tokenController;
        ...
        public void SendSingUpRequest()
        {
            ...
            socketConnection.SendDataToSocket(socketEventManager.SignUpRequstData(), OnSignUpAcknowledgement, LudoNumberEventList.SIGNUP.ToString());
        }
            ...
            try
            {
                ...
                if (signUpRequestSDKData.gameModeName.Equals("NUMBER"))
                {
                    ...
                    if (gamePlayMode != GamePlayMode.Online)
                        signUpAcknowledgement.data.playerMoves = aiController.moves.OrderBy(a => Guid.NewGuid()).ToList();
        ...
        public void CheckForIsKillOrNot()
        {
            ...
            if (signUpRequestSDKData.gameModeName.Equals("NUMBER"))
                if (signUpAcknowledgement.data.thisseatIndex == userTurnStartResponseData.data.startTurnSeatIndex && !userTurnStartResponseData.data.isExtraTurn)
                ...
                for (int i = 0; i < allPlayerHomeController.Count; i++)
                {
                    if (allPlayerHomeController[i].playerInfoData.seatIndex == moveTokenResponse.data.capturedSeatIndex)
                    {
                        if (!signUpRequestSDKData.gameModeName.Equals("CLASSIC"))
                        {
                            for (int j = 0; j < allPlayerHomeController[i].allPlayerToken.Count; j++)
                            {
                                if (allPlayerHomeController[i].allPlayerToken[j].cookieStaticIndex == moveTokenResponse.data.capturedTokenIndex)
                                {
                                    allPlayerHomeController[i].allPlayerToken[j].KillMove();

                                    LudoTokenController coockieMovement = allPlayerHomeController[i].allPlayerToken[j];
            ...
            for (int i = 0; i < joinTableResponse.data.playerInfo.Count; i++)
            {
                for (int j = 0; j < allPlayerHomeController.Count; j++)
                {
                    ...
                    if (gamePlayMode == GamePlayMode.Offline)
                        if (allPlayerHomeController[j].staticSeatIndex != signUpAcknowledgement.data.thisseatIndex)
                            allPlayerHomeController[j].playersMoves = aiController.moves.OrderBy(a => Guid.NewGuid()).ToList();
                        else if (signUpRequestSDKData.gameModeName.Equals("DICE"))
                            signUpAcknowledgement.data.playerMoves = aiController.moves.OrderBy(a => Guid.NewGuid()).ToList();
                }
            }
                    ...
                    for (int j = 0; j < moveTokenResponse.data.updatedScore.Count; j++)
                    {
                        if (gamePlayMode == GamePlayMode.Online)
                            if (allPlayerHomeController[i].playerInfoData.seatIndex == moveTokenResponse.data.updatedScore[j].seatIndex)
                                allPlayerHomeController[i].UpdateUserScore(moveTokenResponse.data.updatedScore[j].score);
                    }
                }
            }
        }
           ...
           tokenController.CoockieManage();
            try
            {
                ...
                if (gamePlayMode == GamePlayMode.Offline)
                {
                    ...
                    {
                        if (signUpAcknowledgement.data.thisseatIndex != userTurnStartResponseData.data.startTurnSeatIndex)
                            aiController.BoatMove();
                    }
                    else
                    {
                        if (signUpAcknowledgement.data.thisseatIndex != userTurnStartResponseData.data.startTurnSeatIndex)
                        {
                            LudoHomeController home = allPlayerHomeController.Find(player => player.staticSeatIndex == userTurnStartResponseData.data.startTurnSeatIndex);
                            home.UpdateDiceValue(home.playersMoves[24 - userTurnStartResponseData.data.movesLeft]);
                            aiController.BoatMove();
                        }
                    }
                }
        ...
        public void LeaveTable(string responseJsonString)
        {
            ...
            for (int i = 0; i < allPlayerHomeController.Count; i++)
            {
                ...
                {
                    allPlayerHomeController[i].UserLeft();
